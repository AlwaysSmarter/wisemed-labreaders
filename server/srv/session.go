...
var provides = make(map[string]ISessionProvider)

func RegisterProvider(name string, provider ISessionProvider) {
	if provider == nil {
...
func (pder *SessionProvider) SessionInit(sid string) (Session, error) {
	pder.lock.Lock()
...
func (pder *SessionProvider) SessionRead(sid string) (Session, error) {
